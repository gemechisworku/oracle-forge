...
# Default: sibling ``DataAgentBench`` clone at repo root (override with DAB_PATH).
DAB_PATH = os.getenv("DAB_PATH") or str(_REPO_ROOT / "DataAgentBench")
...
PG_TOOLS = {
    "query_postgres_oracleforge": {
        "db": "oracleforge",
        "description": (
            "Query the Oracle Forge primary PostgreSQL database (Yelp relational data seeded from DAB; "
            "public schema, business/review/user tables per team scripts)."
        ),
    },
...
MONGO_CONFIG = {
    # Prefer MONGO_URI (Compose); fall back to MONGODB_URI (same as root .env.example / agent).
    "uri": os.getenv("MONGO_URI") or os.getenv("MONGODB_URI", "mongodb://127.0.0.1:27017/"),
}
